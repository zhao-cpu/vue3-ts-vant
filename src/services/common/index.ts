...
class CommonServices extends Services {
	...
	async upload(file: File) {
		...
		const res = await this.instance.post<AxiosData<UploadData>>("/api/upload/image", formData, {
	...
	async dict(type: DictType) {
		const res = await this.instance.get<AxiosData<DictData[]>>("/api/dict", { params: { type } });
		return res.data;
	}
}
